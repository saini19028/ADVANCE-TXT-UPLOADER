...
pwimg = "https://telegra.ph/file/e91729341175f7e6c4e7e.png"
#ytimg = "https://telegra.ph/file/e91729341175f7e6c4e7e.png"
cpimg = "https://i.ibb.co/v6dVWv4h/x.jpg"  
...
OWNER_ID = 7944235618 # Replace with the actual owner's user ID

# List of sudo users (initially empty or pre-populated)
SUDO_USERS = [5876052738]

AUTH_CHANNEL = -1002335878865
...
async def restart_handler(_, m):
    if not is_authorized(m.from_user.id):
        await m.reply_text("**🚫 You are not authorized to use this command. contact:- <b> @Test190224_bot </b>**")
        return
...
async def edit_txt(client, message: Message):
    ...
    try:
        await message.reply_document(
            ...
            caption="📥**𝗘𝗱𝗶𝘁𝗲𝗱 𝗕𝘆 ➤ <b> @SAINI_SAHAB19 </b>**"
        )
    except Exception as e:
...
async def ytplaylist_to_txt(client: Client, message: Message):
    ...
    if videos:
        ...
        await message.reply_document(
            document=file_name, 
            caption=f"`{title}`\n\n📥 𝗘𝘅𝘁𝗿𝗮𝗰𝘁𝗲𝗱 𝗕𝘆 ➤ <b> @SAINI_SAHAB19 </b>"
        )
        os.remove(file_name)
    else:
...
async def help_command(client: Client, msg: Message):
    help_text = (
        "`/start` - Start the bot⚡\n\n"
        "`/saini` - Download and upload files (sudo)🎬\n\n"
    ...
    try:
        ...
        # Check if the response status is OK
            ...
            # Remove the PDF file after sending
                    ...
                    Show = f'**🍁 𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗜𝗡𝗚 🍁**\n\n**📝ɴᴀᴍᴇ » ** `{name}\n\n🔗ᴛᴏᴛᴀʟ ᴜʀʟ » {len(links)}\n\n🗂️ɪɴᴅᴇx » {str(count)}/{len(links)}\n\n🌐ʀᴇᴍᴀɪɴɪɴɢ ᴜʀʟ » {remaining_links}\n\n❄ǫᴜᴀʟɪᴛʏ » {res}`\n\n**🔗ᴜʀʟ » ** `{url}`\n\n🤖𝗕𝗢𝗧 𝗠𝗔𝗗𝗘 𝗕𝗬 ➤ <a href="https://t.me/saini_sahab19">𝙎𝘼𝙄𝙉𝙄 𝙎𝘼𝙃𝘼𝘽 </a>\n\n'
